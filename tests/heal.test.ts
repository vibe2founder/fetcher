  ...
  describe('401 Unauthorized Healing', () => {
    it('deve tentar novamente com timeout aumentado após 401', async () => {
      ...
      try {
        ...
        assert.strictEqual(response.healed, true, 'Deve indicar que foi curado');
        assert.ok(response.healMessage?.includes('Unauthorized'), 'Mensagem deve mencionar Unauthorized');
      } finally {
  ...
  describe('createValueFromType Heuristic', () => {
    it('deve criar valores corretos para diferentes tipos', async () => {
      const testCases = [
        ...
        { message: 'expected uuid', expectedValue: '00000000-0000-0000-0000-000000000000' },
      ];
      ...
      for (const testCase of testCases) {
        ...
        try {
          ...
          assert.strictEqual(response.healed, true, `Deve curar para: ${testCase.message}`);
        } finally {
          global.fetch = originalFetch;
        }
      }
